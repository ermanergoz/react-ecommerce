...
const PopularDepartments = () => {
  ...
  let popularDepartments: PopularDepartmentProps[] = [
    {
      title: t("cooking"),
      productCountText: t("product_count", { PRODUCT_COUNT: 4 }),
      image: img_cooking,
      onClick: () => console.log("Cooking"),
    },
    {
      title: t("fruits"),
      productCountText: t("product_count", { PRODUCT_COUNT: 10 }),
      image: img_fruits,
      onClick: () => console.log("Fruits"),
    },
    {
      title: t("vegetables"),
      productCountText: t("product_count", { PRODUCT_COUNT: 1 }),
      image: img_vegetables,
      onClick: () => console.log("Vegetables"),
    },
    {
      title: t("breakfast"),
      productCountText: t("product_count", { PRODUCT_COUNT: 8 }),
      image: img_breakfast,
    ...
    <div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-2 lg:gap-4 mt-4">
        {popularDepartments.map((popularDepartment) => {
          return (
            <PopularDepartment
              key={popularDepartment.title}
              title={popularDepartment.title}
              productCountText={popularDepartment.productCountText}
              image={popularDepartment.image}
              onClick={popularDepartment.onClick}
            />
          );
        })}
      </div>
